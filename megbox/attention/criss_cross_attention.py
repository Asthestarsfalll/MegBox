...
class CrissCrossAttention(M.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        proj_query_H = proj_query.transpose(
            0, 3, 1, 2).reshape(b*w, -1, h).transpose(0, 2, 1)
        # b,c',h,w -> b,h,c',w -> b*h,c',w -> b*h,w,c'
        proj_query_W = proj_query.transpose(
            0, 2, 1, 3).reshape(b*h, -1, w).transpose(0, 2, 1)
        ...
        energy_H = ((proj_query_H @ proj_key_H) + _build_mask(b, h, w)
                    ).reshape(b, w, h, h).transpose(0, 2, 1, 3)
        ...
        out_H = (proj_value_H @ att_H.transpose(0, 2, 1)
                 ).reshape(b, w, -1, h).transpose(0, 2, 3, 1)
        out_W = (proj_value_W @ att_W.transpose(0, 2, 1)
                 ).reshape(b, h, -1, w).transpose(0, 2, 1, 3)
