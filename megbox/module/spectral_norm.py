...
class SpectralNorm:
    def __init__(
        ...
        if n_power_iterations <= 0:
            raise ValueError(
                "`n_power_iterations` must be positive, but got {}".format(
                    n_power_iterations
                )
            )
    ...
    def compute_weight(self, module: Module, do_power_iteration: bool) -> Tensor:
        ...
        if do_power_iteration:
            for _ in range(self.n_power_iterations):
                v.set_value(
                    normalize(
                        F.matmul(weight_mat, u, transpose_a=True, transpose_b=False),
                        axis=0,
                        eps=self.eps,
                    )
                )
                u.set_value(normalize(F.matmul(weight_mat, v), axis=0, eps=self.eps))
    ...
    def apply(
        ...
        u = mge.Parameter(mge.random.normal(0.0, 1.0, size=[h]))
        v = mge.Parameter(mge.random.normal(0.0, 1.0, size=[w]))
