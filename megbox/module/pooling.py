...
from megengine.functional import max, mean
...
def _calculate_pad_size(h: int, w: int, padding: int, ks: int, stride: int) -> Tuple[int, int]:
    ...
    def _get_windows(self, inp, coords, kernel_size):
        ...
        for h_s, h_e in coords[0]:
            for w_s, w_e in coords[1]:
                windows.append(
                    self.func(inp[:, :, h_s: h_e, w_s: w_e], axis=(2, 3)))
...
def AdaptiveAvgPool2D(
    ...
    return AdaptivePool2D(oshp, mean)


def AdaptiveMaxPool2D(
    oshp: Union[int, Tuple[int, int]],
) -> AdaptivePool2D:
    return AdaptivePool2D(oshp, max)
